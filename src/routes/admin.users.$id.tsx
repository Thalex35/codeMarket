...
function AdminUserDetail() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const [
        ...
        supabase.from("purchases").select("id, amount, currency, status, created_at, software:software_id (name)").eq("user_id", id).order("created_at", { ascending: false }),
  ...
  const totalPaid = paidPurchases.reduce((total, purchase) => total + Number(purchase.amount ?? 0), 0);
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      ...
      <header className="mb-7 flex flex-wrap items-start justify-between gap-4">
        <div><div className="mb-2 flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground"><span>Command Center</span><span>/</span><span>User profile</span></div><h1 className="font-display text-3xl font-bold tracking-tight">{profile.full_name ?? "Unnamed user"}</h1><p className="mt-1 text-muted-foreground">{profile.email}</p></div>
      ...
      <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,7fr)_minmax(250px,3fr)]">
        <main className="min-w-0 space-y-6">
          ...
          <section className="rounded-xl border bg-card" aria-labelledby="billing-heading"><div className="border-b px-5 py-4"><div className="flex items-center gap-2"><Receipt className="h-4 w-4 text-primary" aria-hidden /><h2 id="billing-heading" className="font-semibold">Billing</h2></div><p className="mt-1 text-sm text-muted-foreground">Purchase and payment summary</p></div><div className="p-5"><div className="grid gap-4 sm:grid-cols-3"><div><p className="text-lg font-bold">{totalPaid.toFixed(2)} {currency}</p><p className="text-xs text-muted-foreground">Total paid</p></div><div><p className="text-lg font-bold">{paidPurchases.length}</p><p className="text-xs text-muted-foreground">Completed payments</p></div><div><p className="text-lg font-bold">{purchases.length - paidPurchases.length}</p><p className="text-xs text-muted-foreground">Pending or failed</p></div></div>{!purchases.length ? <p className="mt-5 text-sm text-muted-foreground">No billing activity recorded.</p> : <div className="mt-5 space-y-2">{purchases.slice(0, 4).map((purchase) => <div key={purchase.id} className="flex items-center justify-between gap-3 rounded-lg border p-3"><div><p className="text-sm font-medium">{(purchase.software as unknown as { name: string } | null)?.name ?? "Software purchase"}</p><p className="text-xs text-muted-foreground">{formatDate(purchase.created_at)} · {Number(purchase.amount ?? 0).toFixed(2)} {purchase.currency}</p></div><Badge variant={purchase.status === "paid" ? "outline" : "secondary"} className={purchase.status === "paid" ? "border-success/25 bg-success/10 text-success" : ""}>{purchase.status}</Badge></div>)}</div>}</div></section>
        </main>
